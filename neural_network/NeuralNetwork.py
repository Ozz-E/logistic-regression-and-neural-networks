...
class NeuralNetwork():
    ...
    def forward_pass(self, inputs):
        ...
        # If a forward pass has occurred before (i.e., bias term has been appended to y_hidden), then we have to remove the bias from the hidden neurons
    ...
    def backward_pass(self, inputs, targets, learning_rate):
        ...
        for i in range(self.n_out):
            ...
            error = (outputs - targets) * derivative
            output_deltas = error * sigmoid_derivative(outputs)
        ...
        for i in range(len(hidden_deltas)):
            ...
            hidden_deltas[i] = sigmoid_derivative(self.y_hidden[i])*(self.w_out @ output_deltas)[i]
